...
const UpdatedVehicle = ({ route }) => {
   ...
   useEffect(() => {
      ...
      console.log(vehicle.detail.id, 'vehicle.detail.id');
      console.log(vehicle.detail.id !== id);
   ...
   const onEdit = () => {
      // setChanged({
      //    ...changed,
      //    name: vehicle.detail.name,
      //    idLocation: vehicle.detail.idLocation,
      // });
      setEdit(!edit);
   };
   const onUpdated = () => {
      if (!changed.name) {
         changed.name = vehicle.detail.name;
      }
      if (!changed.idLocation) {
         changed.idLocation = vehicle.detail.idLocation;
      }
      console.log(changed);
      dispatch(updatedVehicle(auth.token, data.id, changed));
   };
   return (
      <>
         {vehicle.isLoading ? (
